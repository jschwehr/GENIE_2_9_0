...
private:
  ...
  double XSecAllTargets(int ,int ,double ,double ,double );
  double TotalXSecAtEAllTargets(int , int , double);
  double interpolateAllTargets(int , int , int , double , double );
  int findPDGHiLo(int,  int& , int& );


  //int fTensorModel; // mec model to use for tensors 1=nieves 2=martini
  //  int fPDDComponent; // generate pdd events? (always, just tag?)

  // other things to add here? hmmm...
